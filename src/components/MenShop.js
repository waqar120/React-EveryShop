...
const MenShop = ({ data, gender }) => {
  ...
  const handleSizeChange = (sizes) => {
    ...
    const filteredProducts = data.filter((product) => {
      const colorFilter =
      selectedColors.length === 0 ||  selectedColors.some(color => product.colors.includes(color.trim()))
      const sizeFilter =
      selectedSizes.length === 0 ||  selectedSizes.some(size =>product.sizes.includes(size.trim()))
      const brandFilter =
      selectedBrand.length === 0 ||  selectedBrand.some(brand =>product.brand.includes(brand.trim()))
  
    return colorFilter && sizeFilter && brandFilter;
  });
